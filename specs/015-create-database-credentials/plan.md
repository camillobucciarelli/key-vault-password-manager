...
| IV — Pixel fidelity is testable | PASS | T015 realises the spec's golden inventory: the two-step wizard at 390×844 light, with 1024×768 and dark named as omitted axes per the spec. The three key modes and the empty-vs-filled password states are covered by named widget assertions rather than a golden per combination, which the constitution permits when the spec names the omitted axes — done in T014. |
