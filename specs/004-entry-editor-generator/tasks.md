...
- [x] **T4** `strength_strip.dart`: radius 20, padding 12/16, 32 circle glyph,
      ...
      pill below.
- [x] **T5** `revealed_password_row.dart`: `attentionTint` bg, mono 16 word-break,
      4 px countdown bar.
- [x] **T6** `RevealController` (12 s): starts on reveal, drains the bar,
      re-masks on expiry, cancelled by a manual toggle. Not disabled by
      `disableAnimations` (the bar animation is; the timer is not).
- [x] **T7** Biometric gate `KvBottomSheet` before reveal when the database has
      biometrics on: "Unlock with biometrics" / "Use password".
- [x] **T8** `totp_row.dart`: 38 circle with seconds 11/700, code mono 21
      +0.16em; 1 s tick, remaining derived from the URI `period`; shares the
      detail screen's single `Ticker` with T6.
- [x] **T9** `lib/core/utils/clipboard_guard.dart`: `copy(value)` writes,
...
- [x] **T10** Wire every copy affordance to `ClipboardGuard` + `KvSnackbar` with
      the existing strings (`Copied password.` etc.), 1600 ms visible.
- [x] **T11** Weak/reused strip variant in `attentionTint` linking to the
      generator, pre-filled with the entry's constraints.

## Editor

- [x] **T12** `vault_entry_editor.part.dart` header: text-only
...
- [x] **T13** Optional list: three rows radius 20, padding 13/16, glyph 17,
      chevron 17 — Custom field, Attachment, One-time code. Tablet variant
      radius 20, padding 12/16, `surfaceNested` on the `surface` pane.
- [x] **T14** Constraint-error states with the two existing literal strings,
      rendered as `KvInput` errors (2 px `accent700`, 12.5 `accent800`).
- [x] **T15** Full-screen QR scanner (`mobile_scanner`, already a dependency) for
      `otpauth://`.
- [x] **T16** Camera-denied screen with the `OTP URI (otpauth://…)` field as the
      manual fallback.
- [x] **T17** Discard-changes `KvBottomSheet`.
- [x] **T18** Saving overlay: keep `isSaving` blocking, card copy
      "Writing to the .kdbx…", backdrop 22 %.

## Generator

- [x] **T19** `vault_generator.part.dart` sheet: gap 16; result box radius 22
...
- [x] **T20** Tablet: generator as the editor's **third column**, same widget,
      no sheet.

## Tablet detail pane

- [x] **T21** Detail pane: copy actions as pills; metadata grid with exactly
      three rows — Created, Updated, last password change.

## Verify

- [x] **T22** String diff vs `strings_004_before.txt` empty.
- [x] **T23** Tests: strength thresholds 40/60/80; TOTP 30→0 with rotation on the
...
- [x] **T24** 13 goldens per the spec table.
- [x] **T25** `flutter analyze` clean, `flutter test` green.
