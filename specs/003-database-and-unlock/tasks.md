...
- [ ] **T2** Add domain `DatabaseFileRepository`, `DatabaseSessionRepository`,
...
- [ ] **T3** Add `DatabaseAccessFailure` and tests mapping absent database,
...
- [ ] **T4** Add `DatabaseSelectionItem`, `DriveAccountSummary` and
...
- [ ] **T6** Add coordinator-owned locate sequencing and BLoC event: accept only a
...
- [ ] **T7** Harden duplicate sequencing: Keep both unique/new ID; Replace dated
      ...
      failure rollback.
- [ ] **T8** Add non-secret `CreateDatabaseStep` to selection state/events.
...
- [ ] **T9** Replace unlock `isLoading` contract with C-4 phase/failure/progress.
...
- [ ] **T10** Extract `welcome_screen.dart`; rebuild selection root/header and
...
- [ ] **T11** Modify/reuse `DatabaseItemTile` for source/biometric/sync metadata,
...
- [ ] **T12** Rebuild Drive loading/empty picker with row geometry,
...
- [ ] **T13** Extract generic `KvBottomSheet.show<T>` from 002 confirmation
...
- [ ] **T14** Add `create_database_screen.dart` with three coordinator-driven
...
- [ ] **T15** Rebuild unlock base for `ready`: 66/r24 feature square, title 32,
...
- [ ] **T17** Render key-selected, biometric-gate and C-4 decrypting states.
...
- [ ] **T18** Replace biometric setup dialog with typed Face ID sheet and rename
...
- [ ] **T19** Add omitted-axis widget matrix from spec: all named state families
...
- [ ] **T20** Add `database_and_unlock_test.dart` with exact 22 filenames/states.
      ...
      22.
- [ ] **T21** Compare post-change copy snapshot; only approved list may differ.
...
- [ ] **T22** Run analyze and all targeted plan commands, then manual matrix. Run
