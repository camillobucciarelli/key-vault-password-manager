...
- [ ] **T0** Snapshot the Italian labels of `browser_setup_screen.dart` and the
...
- [ ] **T2** Theme selector: three equal pills, padding 9, radius 999, selected
      `actionFill`/`actionText`, mapped 1:1 to the three `ThemeCubit` values.
- [ ] **T3** Change-master-password screen: the three existing fields unchanged,
...
- [ ] **T4** `vault_lock_overlay.part.dart`: dark full screen, mark 76 r 24,
...
- [ ] **T5** Privacy overlay: same dark ground, mark only, **zero text nodes**.
...
- [ ] **T6** `autofill_enablement_screen.dart` (iOS): the three system steps plus
      ...
      passwords.
- [ ] **T7** `Link AutoFill credential?` `KvBottomSheet`: Target / Entry /
      Username rows, Reject / Link actions, wired to the existing pending
      association flow unchanged.
- [ ] **T8** `browser_setup_screen.dart`: three step cards radius 24 padding 16 —
      ...
      byte-identical.**
- [ ] **T9** Host-not-found diagnostic naming
      `dev.camillobucciarelli.keyvault_native_host` and the three likely causes.

## Extension

- [ ] **T10** `desktop/browser_extension/tokens.css`: the token subset the popup
      uses, generated from `specs/_design/tokens.css` (same hex values, no
      hand-typed colours).
- [ ] **T11** `popup.html` structure: header 38 (mark 22 r 7, title 12.5/600,
...
- [ ] **T12** `popup.css`: the above geometry on the tokens, width 400,
      `--shadow-md`, plus a `prefers-color-scheme: dark` block using the dark
      mapping.
- [ ] **T13** `popup.js`: the four states — matches (rows radius 14 padding 11/12,
...
- [ ] **T18** `rg -n '#[0-9a-fA-F]{6}' desktop/browser_extension/popup.css` — every
...
- [ ] **T20** `flutter analyze` clean, `flutter test` green.
