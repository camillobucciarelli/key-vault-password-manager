...
function renderResultCard(result, label, canCreatePending) {
  const wrapper = document.createElement("article");
  wrapper.className = "result-card";
...
function renderQueryResults(response) {
  ...
  for (const result of strong) {
    resultsElement.appendChild(renderResultCard(result, "Strong exact host match", false));
  }
...
renderMessage(
  "No password cache",
  "This popup can read app-published metadata and create pending associations. Password reveal/fill remains disabled in native host v2."
);
