...
Regenerating from the SVG is fine — render at 4× and downsample, no additional
padding, no rounding.
...
Also add the files under `flutter: assets:` if the mark is shown inside the UI
(it is: unlock, lock overlay, database list header, tablet rail).
